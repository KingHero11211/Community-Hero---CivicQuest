...
app.get("/api-status", (req, res) => {
  res.json({ status: "CivicQuest API running", model: MODEL, version: "1.0.0" });
});
...
(Submitted via CivicQuest AI Platform)`;
...
async function startServer() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`[CivicQuest Server] Running on http://localhost:${PORT}`);
  });
}
