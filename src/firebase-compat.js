...
console.log(`[CivicQuest Firebase] Config verification: isConfigValid=${isConfigValid}`);
...
try {
  const savedUser = localStorage.getItem("civicquest_user");
  if (savedUser) {
...
function updateAuthUser(user) {
  ...
  if (user) {
    localStorage.setItem("civicquest_user", JSON.stringify(user));
  } else {
    localStorage.removeItem("civicquest_user");
  }
  authListeners.forEach((callback) => {
    try {
      callback(user);
    } catch (err) {
      console.error("[CivicQuest Firebase] Error in auth listener callback:", err);
    }
  });
}
...
function switchToMock(reason) {
  if (!useMockSystem) {
    console.warn(`[CivicQuest Firebase] Switching to local MOCK storage because: ${reason}`);
    useMockSystem = true;
    try {
      const savedUser = localStorage.getItem("civicquest_user");
...
if (isConfigValid) {
  try {
    ...
    realOnAuthStateChanged(realAuthInstance, (user) => {
      ...
      console.warn("[CivicQuest Firebase] Real onAuthStateChanged error, switching to mock:", err);
...
class MockFirestore {
  ...
  getCollection(name) {
    try {
      return JSON.parse(localStorage.getItem(`civicquest_${name}`) || "[]");
    } catch {
      return [];
    }
  }
  ...
  saveCollection(name, data) {
    localStorage.setItem(`civicquest_${name}`, JSON.stringify(data));
...
export async function signInWithDemo() {
  console.log("[CivicQuest Firebase] Using mock user...");
  const dummyUser = {
...
export async function signInWithPopup(authRef, providerRef) {
  console.log("[CivicQuest Firebase] signInWithPopup called", { useMockSystem });
  if (!useMockSystem && authRef && authRef !== mockAuthInstance) {
    try {
      console.log("[CivicQuest Firebase] Attempting real signInWithPopup...");
      const result = await realSignInWithPopup(authRef, providerRef);
      console.log("[CivicQuest Firebase] Real signInWithPopup succeeded!");
      if (result && result.user) {
        updateAuthUser(result.user);
      }
      return result;
    } catch (err) {
      console.error("[CivicQuest Firebase] Real signInWithPopup failed:", err);
      throw err;
    }
  }
...
export async function signOut(authRef) {
  if (!useMockSystem && authRef && authRef !== mockAuthInstance) {
    try {
      ...
      console.error("[CivicQuest Firebase] Real signOut failed:", err);
    }
  }
  updateAuthUser(null);
}
...
export async function updateProfile(userRef, profileData) {
  if (!useMockSystem && userRef) {
    try {
      ...
      console.error("[CivicQuest Firebase] Real updateProfile failed, falling back to mock:", err);
