...
export async function uploadImage(file) {
  ...
  if (!isConfigValid) {
    console.warn("[CivicQuest Upload] Cloudinary credentials not configured. Falling back to local Base64 data URL.");
    try {
      return await fileToBase64(file);
    } catch (err) {
      console.error("[CivicQuest Upload] Failed to read file as Base64, using object URL instead:", err);
  ...
  try {
    ...
    console.error("[CivicQuest Upload] Cloudinary upload failed. Falling back to local Base64 data URL. Error:", err);
    try {
      return await fileToBase64(file);
    } catch (readErr) {
      return URL.createObjectURL(file);
